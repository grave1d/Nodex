...
> Nutze deinen Notion Custom Agent als Gehirn von Codex.
...
Nodex verbindet deinen Notion Custom Agent über eine lokale, OpenAI-kompatible Responses API mit Codex. Agent, Modell, Persona, Anweisungen und Wissen bleiben in Notion. Codex bleibt der lokale Workspace, in dem Dateien, Shell-Befehle, Patches und Tools ausgeführt werden.

**Kein MCP. Keine Remote-Shell. Nodex führt keine Tools aus.** Codex executes tools on your machine; Nodex only bridges requests, responses, and tool-call state.
...
Nodex ist eine lokale Bridge zwischen Notion Custom Agent und Codex. Erstelle einen AI-Agent in Notion, wähle das Modell, schreibe Anweisungen, verbinde Notion mit Nodex und konfiguriere Codex mit Nodex als Custom API Provider.
