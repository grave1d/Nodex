...
> Utilisez votre Notion Custom Agent comme cerveau de Codex.
...
Nodex connecte votre Notion Custom Agent à Codex via une API Responses locale compatible OpenAI. L’agent, le modèle, la personnalité, les instructions et les connaissances restent dans Notion. Codex reste l’espace local où fichiers, commandes shell, patches et outils sont exécutés.

**Pas de MCP. Pas de shell distant. Nodex n’exécute aucun outil.** Codex executes tools on your machine; Nodex only bridges requests, responses, and tool-call state.
...
Nodex est un pont local entre Notion Custom Agent et Codex. Créez un agent IA dans Notion, choisissez le modèle, écrivez les instructions, connectez Notion à Nodex et configurez Codex pour utiliser Nodex comme fournisseur API personnalisé.
