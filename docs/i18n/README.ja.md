...
> Notion Custom Agent を Codex の頭脳として使う。
...
Nodex はローカルで OpenAI 互換の Responses API を通じて Notion Custom Agent と Codex を接続します。エージェント、モデル、ペルソナ、指示、知識は Notion に残ります。Codex はファイル、shell コマンド、パッチ、ツールを実行するローカル workspace のままです。

**MCP 不要。リモート shell ではありません。Nodex はツールを実行しません。** Codex executes tools on your machine; Nodex only bridges requests, responses, and tool-call state.
...
Nodex は Notion Custom Agent と Codex の間にあるローカルブリッジです。Notion で AI Agent を作り、モデルを選び、指示を書き、Notion を Nodex に接続し、Codex に Nodex を custom API provider として設定します。
