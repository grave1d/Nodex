...
> Użyj swojego Notion Custom Agent jako mózgu Codex.
...
Nodex łączy Notion Custom Agent z Codex przez lokalne, zgodne z OpenAI Responses API. Agent, model, persona, instrukcje i wiedza zostają w Notion. Codex pozostaje lokalnym workspace, gdzie wykonywane są pliki, komendy shell, patche i narzędzia.

**Bez MCP. Bez zdalnej powłoki. Nodex nie wykonuje narzędzi.** Codex executes tools on your machine; Nodex only bridges requests, responses, and tool-call state.
...
Nodex to lokalny most między Notion Custom Agent i Codex. Utwórz agenta AI w Notion, wybierz model, napisz instrukcje, połącz Notion z Nodex i skonfiguruj Codex, aby używał Nodex jako custom API providera.
