...
> Використовуйте свого Notion Custom Agent як мозок Codex.
...
Nodex з’єднує ваш Notion Custom Agent із Codex через локальний OpenAI-сумісний Responses API. Агент, модель, персона, інструкції та знання залишаються в Notion. Codex залишається локальним workspace, де виконуються файли, shell-команди, патчі та інструменти.

**Без MCP. Без віддаленої оболонки. Nodex не виконує інструменти.** Codex executes tools on your machine; Nodex only bridges requests, responses, and tool-call state.
...
Nodex — локальний міст між Notion Custom Agent і Codex. Створіть AI-агента в Notion, виберіть модель, напишіть інструкції, підключіть Notion до Nodex і налаштуйте Codex на кастомний API provider.
