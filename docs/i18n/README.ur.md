...
> اپنے Notion Custom Agent کو Codex کا دماغ بنائیں۔
...
Nodex آپ کے Notion Custom Agent کو local, OpenAI-compatible Responses API کے ذریعے Codex سے جوڑتا ہے۔ Agent، model، persona، instructions اور knowledge Notion میں رہتے ہیں۔ Codex وہی local workspace رہتا ہے جہاں files، shell commands، patches اور tools execute ہوتے ہیں۔

**MCP نہیں۔ Remote shell نہیں۔ Nodex tools execute نہیں کرتا۔** Codex executes tools on your machine; Nodex only bridges requests, responses, and tool-call state.
...
Nodex، Notion Custom Agent اور Codex کے درمیان ایک local bridge ہے۔ Notion میں AI Agent بنائیں، model منتخب کریں، instructions لکھیں، Notion account کو Nodex سے connect کریں، اور Codex کو Nodex custom API provider کی طرح configure کریں۔
